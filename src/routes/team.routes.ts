  ...
  try {
    ...
    if (!rateLimit.allowed) {
      return res
        ...
        .json(apiError("RATE_LIMIT", "Too many requests. Please try again in a minute.", undefined, meta()));
    }
    ...
    return res.status(500).json(apiError("SERVER_ERROR", "Unable to load team members right now. Please try again.", undefined, meta()));
  }
});
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      Sentry.captureException(error, { extra: { request_id: requestId } });
      return res.status(500).json(apiError("SERVER_ERROR", "Unable to remove team member right now. Please try again.", undefined, meta()));
    }
  },
);
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      Sentry.captureException(error, { extra: { request_id: requestId } });
      return res.status(500).json(apiError("SERVER_ERROR", "Unable to update team member role right now. Please try again.", undefined, meta()));
    }
  },
);
