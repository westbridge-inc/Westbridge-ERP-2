...
| TD-004 | **`--legacy-peer-deps` used everywhere in CI.** All `npm ci` calls across ci.yml, deploy.yml, security.yml, and release.yml use `--legacy-peer-deps`, masking real dependency conflicts. Root cause is likely a peer dep mismatch between ESLint v9 flat config packages and older plugins.                                                                                                  | Medium   | Medium (2-3 days) | v1.1   | `.github/workflows/ci.yml`                                                 |
| TD-005 | **TruffleHog secrets scan uses `continue-on-error: true`.** The secrets scanning step in security.yml will never fail the pipeline, even if verified secrets are found. This was likely added to unblock CI during initial setup and should be removed once scan is clean.                                                                                                                   | High     | Small (< 1 day)   | v1.0   | `.github/workflows/security.yml:37`                                        |
