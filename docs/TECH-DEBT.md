...
| TD-008 | ~~**`dev-local-session` magic string for local auth fallback.**~~ **RESOLVED** (2026-04-17): Replaced with a `DEV_LOCAL_SESSION` constant.                                                                | ~~Low~~        | ~~Small (< 1 day)~~   | ~~v1.2~~     | Resolved |
