...
| BCR-2 | Are recovery objectives (RTO, RPO) defined?                  | **Y**       | RTO 1h for non-database failures, 4h for full DB restore. RPO ≤24h (daily Tigris backups + WAL archiving when enabled). See BCP §3.                                                                                               |
| BCR-3 | Are backups tested for recoverability on a regular schedule? | **Pending** | Quarterly DR drill scheduled starting 2026-Q2. Backup runbook documented at `docs/runbooks/database-backup.md`.                                                                                                                   |
...
| #      | Question                                                                 | Y/N/NA | Notes                                                                                                                                                                                                         |
| ------ | ------------------------------------------------------------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| IAM-1  | Do you support Multi-Factor Authentication (MFA)?                        | **Y**  | TOTP (RFC 6238) MFA is available to all users. Backup codes issued at TOTP enrollment. Implementation: `src/routes/totp.routes.ts`, `src/lib/totp.ts`.                                                        |
...
| IAM-3  | Are passwords stored using a one-way hash function?                      | **Y**  | bcrypt with cost factor ≥10 (`src/lib/services/auth.service.ts`). Plaintext is never stored.                                                                                                                  |
| IAM-4  | Are password complexity requirements enforced?                           | **Y**  | Minimum 12 characters, at least one letter and one number. Validated by `src/lib/password-policy.ts`.                                                                                                         |
...
| IAM-8  | Are session tokens cryptographically random?                             | **Y**  | 64-byte random session tokens (`crypto.randomBytes(64)`). Stored as SHA-256 hashes in the database.                                                                                                           |
