...
- **Local password:** bcrypt hash with cost factor ≥10. Plaintext is never stored. Minimum length 12 characters; must contain at least one letter and one number. Validated by `src/lib/password-policy.ts`.
- **Multi-factor authentication:** TOTP (RFC 6238) is **available to all users and required for owner/admin roles on every account by policy**. Backup codes are issued at TOTP enrollment and consumed once. Implementation: `src/routes/totp.routes.ts`, `src/lib/totp.ts`.
- **Single sign-on:** SAML 2.0 and OIDC supported via `src/routes/sso.routes.ts` + `src/lib/services/sso.service.ts`. SSO configuration is per-account and stored encrypted at rest.
- **Account lockout:** After 5 consecutive failed login attempts, the account is locked for 15 minutes. Lockout state is enforced in `src/lib/services/auth.service.ts` via `failedLoginAttempts` and `lockedUntil` columns.
...
- **Session lifetime:** 30 days, sliding window. Session tokens are 64-byte random values, stored as SHA-256 hashes in the database with the AES-256-GCM-encrypted ERPNext SID. See `src/lib/services/session.service.ts`.
- **Session revocation:** Triggered automatically on password change (preserves the current session, revokes all others — see `auth.service.ts:158-166`); manually via `revokeSession` and `revokeAllUserSessions`.
